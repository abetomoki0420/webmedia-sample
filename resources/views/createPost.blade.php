...
@section('content')
  ...
  <form class="siimple-form" action="{{action('MediaController@post')}}" method="post" enctype="multipart/form-data">
    ...
    <div class="siimple-field">
      ...
      <input class="siimple-input siimple-input--fluid" type="text" name="title" value="{{ old('title') }}" required></input>
      @if( $errors->has('title'))
    ...
    <div class="siimple-field">
      ...
      <textarea class="siimple-textarea siimple-textarea--fluid" name="body" rows="5" required>{{old('body')}}</textarea>
      @if( $errors->has('body'))
